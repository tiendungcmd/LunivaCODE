...
<body>
								...
								<td align="left" width="80px"><select name="group_id">
										...
										<c:forEach items="${lstGroup}" var="list">
											<c:if test="${group_id == list.group_id }">
												<option value="${list.group_id}" selected>${fn:escapeXml(list.group_name)}</option>
											</c:if>
											<c:if test="${group_id != list.group_id }">
												<option value="${list.group_id}">${fn:escapeXml(list.group_name)}</option>
											</c:if>
										</c:forEach>
								</select></td>
								<td align="left">
								<input class="btn" type="submit" name="search" value="検索" /> 
								<input class="btn" type="button"  onclick='window.location.href="AddUserInput.do"' value="新規追加" />
								</td>
							</tr>
						</table>
					</td>
	...
	<c:if test="${lstUser.size()!=0 }">
		...
		<table class="tbl_list" border="1" cellpadding="4" cellspacing="0"
			...
			<tr class="tr2">
				...
				<th align="left">氏名 <a href="${sortFullName }"><c:if
							...
							test="${sortByFullName == 'ASC' }">▲▽</c:if> </a>
				</th>
		...
		<table>
			<tr>
				<c:set var="firstPage" value="${lstPaging[0]}" />
				<c:set var="lastPage" value="${lstPaging[2]}" />
				...
				<c:if test="${firstPage > limitPage}">
					<a href="${paging}&currentPage=${firstPage-limitPage}">
						&lsaquo;
						&lsaquo;
					</a>
					&nbsp;
				...
				<c:forEach items="${lstPaging }" var="page">
			
					<c:if test="${currentPage == page }">${page }</c:if>
					<c:url var="pagingUrl" value="ListUserController">
						<c:param name="action" value="paging"></c:param>
						<c:param name="sortType" value="${sortType }"></c:param>
						<c:param name="fullName" value="${fullName }"></c:param>
						<c:param name="group_id" value="${group_id }"></c:param>
					...
					<c:if test="${currentPage != page }">
						<a href="${pagingUrl }">${page }</a>
						&nbsp;
					</c:if>
				</c:forEach>
				<c:if test="${!lstPaging.contains(totalPage)}">
					<a href="${paging}&currentPage=${firstPage+limitPage}">
						&rsaquo;
						&rsaquo;
					</a>
