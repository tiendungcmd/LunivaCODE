		...
		try {
			...
			if (conn != null) {
				...
				StringBuilder sql = new StringBuilder(
						"SELECT count(user_id) FROM tbl_user u  inner join mst_group g on u.group_id=g.group_id where u.rule= ? ");
		...
		try {
			...
			if (conn != null) {
				...
				if (sortType.equals("full_name")) {
					sql.append(" ORDER BY u.full_name " + sortByFullName + ", de.code_level "
							+ Constant.SORT_NAME_LEVEL_DEFAULT + ", de.end_date " + Constant.SORT_END_DATE_DEFAULT);
				} else if (sortType.equals("code_level")) {
					sql.append(" ORDER BY de.code_level " + sortByCodeLevel + ", u.full_name "
							+ Constant.SORT_FULL_NAME_DEFAULT + ", de.end_date " + Constant.SORT_END_DATE_DEFAULT);
				} else if (sortType.equals("end_date")) {
					sql.append(" ORDER BY de.end_date " + sortByEndDate + ", u.full_name "
							+ Constant.SORT_FULL_NAME_DEFAULT + ", de.code_level " + Constant.SORT_NAME_LEVEL_DEFAULT);
				}
